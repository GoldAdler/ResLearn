...
public class ViewAufgabeLaden extends Application {

	private Pane pane;
	File f = new File("./eigeneAufgaben");
	...
	private static ViewAufgabeLaden view;

	public static ViewAufgabeLaden getInstance() {
		if (view == null) {
			view = new ViewAufgabeLaden();
		}
		return view;
	}
	...
	public Pane erstellePane() {
		...
		if (fileArray.length == 0) {
			...
			info.setText("Sie haben noch keine eigenen Aufgaben erstellt.");
			info.setStyle("-fx-font:" + DisplayCanvas.schriftGroesse + " Arial;");
			info.setLayoutX(DisplayCanvas.buttonXStart / 2);
			info.setLayoutY(DisplayCanvas.buttonYStart / 2);
	...
	public void weiter(ActionEvent event) {
		...
		Controller.alleFenster.add("/reslearn/gui/fxml/AufgabeLaden.fxml");
		try {
